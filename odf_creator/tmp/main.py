...
from odf.text import P, H, A, S, List, ListItem, ListStyle, ListLevelStyleBullet, ListLevelStyleNumber, \
    ListLevelStyleBullet, Span, UserFieldGet
...
StandardStyle.addElement(TextProperties(attributes={'fontsize':"24pt",'fontweight':"bold" }))
...
item = ListItem()
item.addElement(P(text='figure out how to apply what we learned to spreadsheets andpresentations', stylename="P5"))
textList.addElement(item)
...
h_elem = H(outlinelevel=1)

h_elem.addElement(UserFieldGet(name="name1"))
textdoc.text.addElement(h_elem)

h_elem.addElement(UserFieldGet(name="name2"))
textdoc.text.addElement(h_elem)
# textdoc.text.addElement(H(outlinelevel=1, text='An Image', id='asd'))

p = P()
textdoc.text.addElement(p)

texts = textdoc.getElementsByType(text.H)
s = len(texts)
for i in range(s):
    el = texts[i]
    # el = text.H()
    try:
        if el.getAttribute("id") == 'asd':
            # el.setAttribute("text", "Хуйнаны")
            print(el)
